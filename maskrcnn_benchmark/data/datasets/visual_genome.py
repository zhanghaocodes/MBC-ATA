...
class VGDataset(torch.utils.data.Dataset):
    ...
    def get_groundtruth(self, index, evaluation=False, flip_img=False):
        ...
        for i in range(relation.shape[0]):  #对每一个关系进行遍历
...
def load_graphs_Resample(roidb_file, split, num_im, num_val_im, filter_empty_rels, filter_non_overlap):
    """
    Load the file containing the GT boxes and relations, as well as the dataset split
    Parameters:
        roidb_file: HDF5
    ...
    cls_cur_num = []
    for i in range(51):
        cls_cur_num.append(0)
    cls_tar_num = [807.0, 807.0, 807.0, 807.0, 807.0, 807.0, 807.0, 807.0, 807.0, 807.0, 807.0, 807.0, 807.0,
                        807.0, 807.0, 807.0, 807.0, 807.0, 807.0, 807.0, 807.0, 807.0, 807.0, 807.0, 807.0, 807, 779,
                        702, 679, 652, 641, 580, 512, 511, 493, 485, 435, 369, 343, 327, 289, 265, 263, 224, 198, 172,
                        153, 134, 128, 49, 5]
    # cls_tar_num = [807.0, 807.0, 807.0, 807.0, 807.0, 807.0, 807.0, 807.0, 807.0, 807.0, 807.0, 807.0, 807.0,
    #                     807.0, 807.0, 807.0, 807.0, 807.0, 807.0, 807.0, 807.0, 807.0, 807.0, 807.0, 807.0, 807, 10000,
    ...
    for i in range(len(image_index)):  #每次循环就是处理一张img
        i_obj_start = im_to_first_box[i]
        i_obj_end = im_to_last_box[i]
        i_rel_start = im_to_first_rel[i]
        i_rel_end = im_to_last_rel[i]
        ...
        boxes_i = all_boxes[i_obj_start : i_obj_end + 1, :]
        gt_classes_i = all_labels[i_obj_start : i_obj_end + 1]  #每个box的class
        ...
        if i_rel_start >= 0:
            ...
            if split == 'train':#筛选工作
                ...
                for index in range(len(predicates)):
                    if cls_cur_num[predicates[index]] < cls_tar_num[predicates[index]]:
                        flag = True
                        cls_cur_num[predicates[index]] +=1
                        tmp_pre_list.append(predicates[index])
                        tmp_obj_list.append(obj_idx[index])
                if flag:
                    realTrainNum += 1
                if not flag:
                    split_mask[image_index[i]] = 0
                    continue
                obj_idx = tmp_obj_list
                predicates = tmp_pre_list
                rels = np.column_stack((obj_idx, predicates))  # (num_rel, 3), representing sub, obj, and pred
                #resample 2.0
                # tmp_pre_list = []
                # tmp_obj_list = []
        ...
        if filter_non_overlap:   #train 的话是True
            assert split == 'train'
            # construct BoxList object to apply boxlist_iou method
            # give a useless (height=0, width=0)
            boxes_i_obj = BoxList(boxes_i, (1000, 1000), 'xyxy')
...
def load_graphs_Calculate(roidb_file, split, num_im, num_val_im, filter_empty_rels, filter_non_overlap):
    """
    Load the file containing the GT boxes and relations, as well as the dataset split
    Parameters:
        roidb_file: HDF5
